...
public abstract class RNRfidOrca50Thread extends Thread {
	...
	public boolean connect() {
		...
		try {
			...
			// จับ Throwable ไม่ใช่แค่ Exception — เครื่องที่ไม่ใช่ Orca 50 ตัวจริง (เช่น emulator
			// arm64, มือถือรุ่นอื่น) จะไม่มี libserial_port.so/libmodule_manager.so ที่ตรง ABI
			// ทำให้ System.loadLibrary() throw UnsatisfiedLinkError ซึ่งเป็น Error ไม่ใช่
			// Exception — ถ้าจับแค่ Exception แอปทั้งตัวจะ crash แทนที่จะ fail เฉยๆ
